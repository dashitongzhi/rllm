...
class VerlBackend(BackendProtocol[Iterable, DataProto]):
    ...
    def transform_to_backend_batch(self, trainer_state: TrainerState, **kwargs) -> DataProto:
        ...
        if trainer_state.episodes is not None:
            batch = transform_episodes_to_dataproto(trainer_state.episodes, self.rollout_engine, max_prompt_length, max_response_length)
        ...
        batch = transform_trajectory_groups_to_dataproto(trainer_state.trajectory_groups, self.rollout_engine, max_prompt_length, max_response_length)
